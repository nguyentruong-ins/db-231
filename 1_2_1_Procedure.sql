...
BEGIN 
	...
	if @ssn IS NOT NULL
	BEGIN
		SELECT @count = COUNT(*) from employee_accounts where ssn = @ssn and username != @username;
		if @count > 0
		BEGIN
			SET @isError = 1;			
			RAISERROR ('Duplicated ssn', 16, 1);	
	...
	BEGIN		
		UPDATE employee_accounts SET 
			...
			sex = ISNULL(@sex, sex),
			started_date = ISNULL(@started_date, started_date),
			ssn = ISNULL(@ssn, ssn),
			employee_type = ISNULL(@employee_type, employee_type),
			store_id = ISNULL(@store_id, store_id),
...
@username = 'nguyen.truong5', 
@password = '1234569999',
@salary = 1999999,
@dob = '2003/11/12',
@employee_type = 'FULLTIME',
@account_type = 'EMPLOYEE',
@old_number = '0945430555',
